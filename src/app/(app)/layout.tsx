...
import { AlertasCampana } from "@/components/alertas-campana";
import { AlertasSlot } from "@/components/alertas-slot";
...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  // Campanita en un Suspense: sus consultas no bloquean ni compiten en el camino
  // crítico de la página (el shell pinta ya; el contador llega un instante después).
  const alertasSlot = empresaIdActiva ? (
    <Suspense fallback={<AlertasCampana items={[]} />}>
      <AlertasSlot empresaId={empresaIdActiva} permisos={permisos} />
    </Suspense>
  ) : (
    <AlertasCampana items={[]} />
  );
